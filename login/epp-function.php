...
class dpis extends ociConn 
{
    ...
    function queryPersonal($per_cardno) {   //ใช้กับไฟล์ ajax-modal-move_person-show.php view-load-person2.php  
        $success = array();
        $ociSql = "SELECT
                    per_personal.per_id,
    ...
    function updatePer_Personal($data) {
        ...
        $err = '';
        try{
            $sql = "UPDATE `per_personal_2016`   SET 
                                                    `per_id` = :per_id ,  
                                                    `pn_name` = :pn_name , 
                                                    `per_name` = :per_name , 
    ...
    function insertPer_Personal($data) {
        ...
        // echo "</pre>";
        try{
            $sql = "INSERT INTO `per_personal_2016` (
                                                    `per_id`, 
                                                    `per_cardno`, 
                                                    `pn_name`, 
                                                    `per_name`, 
