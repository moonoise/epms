<?php
 if (!empty($_GET['kpi_code']) && !empty($_GET['per_cardno']) && !empty($_GET['kpi_score_id'])) {
    ...
    // $kpiScoreTable  $per_personalTable
    $checkScore = $kpi->kpiBtnStatus1($_GET['kpi_score_id'],$kpiScoreTable);

    if ($checkScore['success'] === true) {
        $scoreDisabled  = 'Disabled';
        $acceptDisabled = '';

        $checkaccept = $kpi->kpiBtnStatus2($_GET['kpi_score_id'],$kpiScoreTable);
    ...
    $t .= "<table id='table-cpc-sub' class='table table-hover table-bordered' style='width:100%'>
        ...
        if ($cComment > 0 ) {
            ...
            foreach ($rComment as $key => $value) {
                $h = $kpi->WhoIsHead($value['kpi_score_id'],$per_personalTable,$kpiScoreTable); //$kpiScoreTable  $per_personalTable
                // echo "<pre>";
                // print_r($h);
                // echo "</pre>";
                $t .= "<tr>";
