...
class cpc extends DbConn
{
    ...
    function cpcScoreSelect($data, $cpcScoreTable)
    {  // ใช้กับไฟล์ ajax-modal-cpc_score-show.php และ ajax-modal-clear_score.php
        $err = '';
        $success = array();
        try {
            $sql = "SELECT
    ...
    function cpcScoreGetDefault($per_cardno)
    {
        $err = '';
        $success = array(
            'success' => null,
        ...
        try {
            $cpc_question_create = "SELECT * FROM cpc_question_create";
    ...
    //ค่าคาดหวัง เปลี่ยนเป็น  ค่าน้ำหนัก
    function cpc_divisor($question_no)
    {
        $err = '';
        $success = array();
        try {
            $sql = "SELECT weight_default FROM cpc_question WHERE question_no = :question_no";
            $stm = $this->conn->prepare($sql);
            ...
            $success['success'] = true;
            $success['msg'] = null;
            $success['weight_default'] = $r[0]['weight_default'];
        } catch (Exception $e) {
            $success['msg'] = $e->getMessage();
            $success['success'] = null;
            $success['weight_default'] = null;
        }
    ...
    function cpcBtnStatus1($cpc_score_id, $cpcScoreTable)
    {  //รอยืนยัน
        $err = '';
        $success = array();
        try {
    ...
    function cpcBtnStatus2($cpc_score_id, $cpcScoreTable)
    {  //เช็คว่าผู้บังคับบัญชายืนยันครบยัง
        $err = '';
        $success = array();
        try {
            $sql = "SELECT CASE 
    ...
    function cpcStatus3($per_cardno, $years, $cpcScoreTable)
    {  // เช็ดว่า  ผู้ประเมิน ทำประเมินครบแล้วยัง  return ออกมาเป็นจำนวนที่ประเมิน และจำนวนเป็นเสร็จแล้ว และจำนวนที่ Accept แล้ว
        $err = '';
        $success = array();
        $choiseFinish = 0;
        $choiseAccepted = 0;
    ...
    function cpcStatus4($per_cardno, $years)
    { //เช็ดว่า  ผู้ประเมิน ทำประมาณครบแล้วยัง  return true / false
        $success = array();
