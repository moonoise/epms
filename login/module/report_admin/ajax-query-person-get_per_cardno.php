...
try {
    if ($orgSelect == "77") {
        $sql = "SELECT `per_cardno` FROM $personalTable WHERE `login_status` = 1 AND per_type = 3";
        $stm = $db->conn->prepare($sql);
        $stm->execute();
    } else {
        $sql = "SELECT  `per_cardno` FROM $personalTable WHERE (`org_id` = :org_id OR `org_id_1` = :org_id OR `org_id_2` = :org_id) AND `login_status` = 1 AND per_type = 3 ";
        $stm = $db->conn->prepare($sql);
...
try {
    if ($orgSelect == "77") {
        ...
        $sqlLoginStatus = "SELECT  `per_cardno` FROM $personalTable WHERE `login_status` = 0 AND per_type = 3 ";
        $stmLoginStatus = $db->conn->prepare($sqlLoginStatus);
        $stmLoginStatus->execute();
    } else {
        $sqlLoginStatus = "SELECT  `per_cardno` FROM $personalTable WHERE (`org_id` = :org_id OR `org_id_1` = :org_id OR `org_id_2` = :org_id) AND `login_status` = 0  AND per_type = 3";
        $stmLoginStatus = $db->conn->prepare($sqlLoginStatus);
